...
def setText(text):
    ...
    '''
    resultText.insert(tkinter.END,text)
    resultText.see(tkinter.END)
    ...
    def selectremove():
        ...
        if name:
            for j,i in enumerate(name):
                var.append(tkinter.IntVar(remove))
                tkinter.Checkbutton(remove, text=i, variable=var[j]).grid(row=j//2,column=j%2)
        ...
        tkinter.Button(remove, text='Kill', command=autoremove_kill, width=20).grid(row=len(name)//2+1, column=0 , pady=4)
        tkinter.Button(remove, text='Remove', command=autoremove_remove, width=20).grid(row=len(name)//2+1, column=1, pady=4)
...
def run(path='',fA='',fB='',fC='',lx=0.0):
    ...
    def runonline():
        ...
        if result[0]=='1' or result[0]=='2':
            sshcommand("cp ./update/in.d " + filepath + "/in.d")
...
def downloadmax(pathlist, variable):
    '''
    Download the freeE in printout.txt from paths in pathlist and save to a file in ./freeE/
    File name is generated according to fA, fB, fC and lx
    '''
    ...
    for i, path in enumerate(pathlist):
        print(path)
        # message=sshcommand("cat "+ path +"/pha.dat|sed -n '8,8p'|cut -c 19-26")
        message=sshcommand("cat "+ path +"/printout.txt|tail -1|cut -c 17-30")
        setText("MAXDOWNLOADED\n")
        try:
            float(message)
            variablefloat.append(float(variable[i]))
            energyfloat.append(float(message))
    ...
    pyplot.scatter(variablefloat,energyfloat)
    
    func = interpolate.interp1d(variablefloat,energyfloat,kind='quadratic')
    x = np.linspace(min(variablefloat), max(variablefloat), 100)
    y = func(x)

    pyplot.plot(x,y)
    setText(str(variablefloat)+'\n')
    setText(str(energyfloat)+'\n')
...
def openmultirun():
    ...
    mrframe=tkinter.Frame(mrroot)
    v=tkinter.IntVar(mrroot)
    v.set(0)

    tkinter.Radiobutton(mrframe,text='fA',variable=v,value=0).grid(row=0,column=0,padx=5)
    tkinter.Radiobutton(mrframe,text='fB',variable=v,value=1).grid(row=0,column=1,padx=5)
    tkinter.Radiobutton(mrframe,text='fC',variable=v,value=2).grid(row=0,column=2,padx=5)

    entryfa=tkinter.Entry(mrframe)
    entryfa.grid(row=1,column=0)
    entryfb=tkinter.Entry(mrframe)
    entryfb.grid(row=1,column=1)
    entryfc=tkinter.Entry(mrframe)
    ...
    def ultrarun():
        ...
        def ultrarunonline():
            ...
            lxrange=eval('list(np.arange('+rangeinput.get().strip()+'))')
            sleeptime=int(sleeptimeinput.get().strip())
    ...
    btframe=tkinter.Frame(mrroot)
    tkinter.Button(btframe,text='run',command=multirun,width=10).grid(row=0,column=0,padx=5)
    tkinter.Button(btframe,text='getresult',command=getresult,width=10).grid(row=0,column=1,padx=5)
    tkinter.Button(btframe,text='ultrarun',command=ultrarun,width=10).grid(row=0,column=2,padx=5)
    tkinter.Button(btframe,text='ultraresult',command=ultraresult,width=10).grid(row=0,column=3,padx=5)
    tkinter.Button(btframe,text='ultramax',command=ultramax,width=10).grid(row=0,column=4,padx=5)
    tkinter.Label(btframe,text='range',width=10).grid(row=1,column=0,padx=5)
    tkinter.Label(btframe,text='sleeptime',width=10).grid(row=1,column=2,padx=5)
    rangeinput=tkinter.Entry(btframe,width=10)
    rangeinput.grid(row=1,column=1,padx=5)
    sleeptimeinput=tkinter.Entry(btframe,width=10)
    sleeptimeinput.grid(row=1,column=3,padx=5)
...
def openmultilx():
    ...
    mrframe=tkinter.Frame(mrroot)
    tkinter.Label(mrframe,text='lx/lx0').grid(row=0,column=0,padx=5)

    entrylx=tkinter.Entry(mrframe)
    ...
    btframe=tkinter.Frame(mrroot)
    tkinter.Button(btframe,text='run',command=multilx,width=10).grid(row=0,column=0,padx=5)
    tkinter.Button(btframe,text='getresult',command=getresult,width=10).grid(row=0,column=1,padx=5)
...
def connectcdy():
    global transport, sftp, ssh

    if transport and sftp and ssh:
        transport.close()
        sftp.close()
        ssh.close()
...
if __name__ == '__main__':
    ...
    for i in range(len(labels)):
        entry.append(tkinter.Entry(frame))
        tkinter.Label(frame, text=labels[i]).grid(row=i//3,column=(i%3)*2)
        entry[i].insert(10,defaults[i])
        entry[i].grid(row=i//3,column=(i%3)*2+1)

    buttonframe = tkinter.Frame(root)
    tkinter.Button(buttonframe, text='generate',command=generate,width=15).grid(row=0,column=0,padx=5,pady=5)
    tkinter.Button(buttonframe, text='update',command=update,width=15).grid(row=0,column=1,padx=5,pady=5)
    tkinter.Button(buttonframe, text='run',command=run,width=15).grid(row=0,column=2,padx=5,pady=5)
    tkinter.Button(buttonframe, text='multirun',command=openmultirun,width=15).grid(row=0,column=3,padx=5,pady=5)
    tkinter.Button(buttonframe, text='multilx',command=openmultilx,width=15).grid(row=0,column=4,padx=5,pady=5)
    tkinter.Button(buttonframe, text='top',command=top,width=15).grid(row=1,column=0,padx=5,pady=5)
    tkinter.Button(buttonframe, text='getpha',command=getfile,width=15).grid(row=1,column=1,padx=5,pady=5)
    tkinter.Button(buttonframe, text='classify',command=classify,width=15).grid(row=1,column=2,padx=5,pady=5)
    tkinter.Button(buttonframe, text='mayavi',command=mayavi,width=15).grid(row=1,column=4,padx=5,pady=5)
    tkinter.Button(buttonframe, text='printout',command=printout,width=15).grid(row=0,column=5,padx=5,pady=5)
    tkinter.Button(buttonframe, text='autoremove',command=autoremove,width=15).grid(row=1,column=3,padx=5,pady=5)
    tkinter.Button(buttonframe, text='connectcdy',command=connectcdy,width=15).grid(row=0,column=6,padx=5,pady=5)
    tkinter.Button(buttonframe, text='connectnxw',command=connectnxw,width=15).grid(row=1,column=6,padx=5,pady=5)
    btmatlab = tkinter.Button(buttonframe, text='matlab',command=matlab,width=15)

    nodeframe = tkinter.Frame(root)
    nodeVar=tkinter.StringVar(root)
    nodeVar.set('c0103')
    nodes=['c0102','c0103','c0105','c0106','c0107','c0108','c0109','c0110','c0111','c0112','c0113']
    for i, item in enumerate(nodes):
        tkinter.Radiobutton(nodeframe, text=item, variable=nodeVar, value=item,command=vmstat).grid(row=0,column=i,padx=5)

    resultText = tkinter.Text(root,height=20,width=85)
    cmdEntry = tkinter.Entry(root,width=96)
